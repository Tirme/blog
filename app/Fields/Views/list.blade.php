<section ng-controller="ListController" ng-init="init('{{$model_name}}', {{{json_encode($columns)}}})">
    <div class="panel panel-info">
                            ...
                            @foreach ($columns as $key => $column)
                                <li>
                                    <a href="javascript: void(0);">
                                        <input type="checkbox" id="columns['{{$key}}']" ng-model="columns['{{$key}}']" />
        ...
        @endif
        <table class="table table-hover" ng-if="inited">
            <thead>
                <tr class="info">
                    @foreach ($columns as $key => $column)
                        <th ng-show="columns['{{$key}}']">{{$column}}</th>
                    @endforeach
                    <th>Created At</th>
                    <th>Updated At</th>
                    <th>Actions</th>
            ...
            <tbody>
                @forelse ($rows as $row)
                    <tr>
                        @foreach ($columns as $key => $column)
                            <td ng-show="columns['{{$key}}']">{!!$row->$key->getContent()!!}</td>
                        @endforeach
